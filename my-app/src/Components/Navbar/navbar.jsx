...
export function NavbarComponent() {
  ...
  const HandleOnClick1 = () => navigate("/Login");
  const HandleOnClick2 = () => navigate("/Signup");
...
<Navbar className="fixed top-0 left-0 right-0 z-50 !bg-white border-b shadow-md px-4 py-3 transition-all duration-300">
      <div className="container mx-auto flex flex-wrap items-center justify-between">
        {/* Logo */}
        <NavbarBrand>
          <img src={logo} className="h-13 w-30  object-contain  " alt="" />
        </NavbarBrand>
          ...
          <nav className="flex flex-col md:flex-row gap-3 md:gap-3">
            {["/", "About","Services", "Shop", "Contact" ].map((route, i) => (
              <NavLink
                ...
                onClick={closeMenu}
                className={({ isActive }) =>
                  `block px-4 py-2 rounded-md text-md font-semibold transition-all duration-200 ${
                    isActive
